...
Garner.prototype.searchFor = function(searchURL, compareValue, max, excludes) {
	...
	if (searchURL) {
		...
		return new Promise(function(resolve, reject) {
			bakajax.post(self.url+'/get/'+self.databaseId+'/'+self.albumId+'/'+self.username, {'sessionKey': self.sessionKey, 'searchURL': searchURL, 'compareValue': compareValue, 'max': max, 'excludes': excludes})
			.then(function(response) {
...
Garner.prototype.updateItem = function(searchURL, compareValue, replacementURL, replacementItem) {
	...
	if (searchURL && (compareValue || compareValue == '') && replacementURL && (replacementItem || replacementItem == '')) {
		...
		return new Promise(function(resolve, reject) {
			bakajax.put(self.url+'/'+self.databaseId+'/'+self.albumId+'/'+self.username, {'sessionKey': self.sessionKey, 'searchURL': searchURL, 'compareValue': compareValue, 'replacementURL': replacementURL, 'replacementItem': replacementItem})
			.then(function(response) {
				switch (response.code) {
					case 200:
						resolve(response.data);
